...
public class ChildFilter implements CowFilter {

    private Cow parent;

    public ChildFilter (Cow p) {
        this.parent = p;
    }

    @Override
    public String toString() {
        return "Children of " + parent.getID();
    }

    @Override
    public boolean meetsCriteria(Cow cow) {
        if (cow.getDam() == parent || cow.getSire() == parent) {
            return true;
        }
        else {
            return false;
